...
class LinkedHabit(models.Model):
    ...
    is_pleasurable = models.BooleanField(default=True, verbose_name="определяет является привычка хорошей или нет")
...
class Habit(models.Model):
    ...
    place = models.ForeignKey(Place, null=True, blank=True, on_delete=models.SET_NULL, verbose_name="Место")
    ...
    pleasant_habit = models.ForeignKey(PleasantHabit, null=True, blank=True, on_delete=models.SET_NULL, verbose_name="Приятная привычка")
    linked_habit = models.ForeignKey(LinkedHabit, null=True, blank=True, on_delete=models.SET_NULL, verbose_name="Связанная привычка")
    ...
    estimated_time = models.PositiveIntegerField(verbose_name="Оцененное время выполнения")
