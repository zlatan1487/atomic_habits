...
def validate_linked_habit_is_pleasurable(linked_habit):
    """
       ...
       :param linked_habit: Связанная привычка, которую необходимо проверить.
       :type linked_habit: LinkedHabit or None
       :raises serializers.ValidationError: Если связанная привычка не является приятной.
    """
    if linked_habit:
...
def validate_reward_and_linked_habit(data):
    """
    Проверяет, что не выбрано одновременно и вознаграждение, и связанная привычка.
    """
    has_reward = data.get('reward')
    linked_habit = data.get('linked_habit')

    if has_reward and linked_habit:
        raise serializers.ValidationError("Выберите либо вознаграждение, либо связанную привычку, но не оба.")


def validate_pleasant_habit(data):
    reward = data.get('reward')
    linked_habit = data.get('linked_habit')
    print('linked-------', linked_habit)
    if reward is not None or linked_habit is not None:
        raise serializers.ValidationError(
            "У Приятной привычке не может быть ни вознаграждения, ни связанной привычки.")
...
def validate_execution_frequency(data):
    ...
    if frequency and last_completed:
        # Вычисляем разницу между последним выполнением привычки и текущим временем
        time_difference = timezone.now() - last_completed

        # Проверяем, что разница во времени больше или равна 7 дням
